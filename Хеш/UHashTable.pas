...
function hashSecond(key: TKey): integer;
begin
  Result :=Trunc(TABLE_SIZE*Frac((A*key)));
end;

function THashTable.hash(key: TKey; i:Integer): integer;
begin
  Result :=(key mod TABLE_SIZE + i*hashSecond(key))mod TABLE_SIZE;
end;
...
begin
  ...
  for i:=0 to TABLE_SIZE - 1 do
    if table[i] <> nil then
      begin
        writeln(f,table[i].value.studentId);
        writeln(f, table[i].value.FIO);
        writeln(f,table[i].value.sessionResult.failedTests);
        writeln(f,table[i].value.sessionResult.failedExams);
      end;
  Close(f);
end;
...
begin
  ...
  while not Eof(f) do
    begin
      value := TValue.Create();
      readln(f, value.studentId );
      readln(f,value.FIO);
      readln(f, value.sessionResult.failedTests);
      readln(f, value.sessionResult.failedExams);
      add(value.studentId, value);
      inc(result);
    end;
  Close(f);
end;
    ...
    if self.table[i] <> nil then
      with(self.table[i].value) do
        begin
          ...
          table.Cells[0,rowIndex] := IntToStr(studentId);
          table.Cells[1,rowIndex] := FIO;
          table.Cells[2,rowIndex] := IntToStr(sessionResult.failedTests);
          table.Cells[3,rowIndex] := IntToStr(sessionResult.failedExams);
